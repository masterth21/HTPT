...
public class FavoritesService {
	...
	@Autowired
	FavoritesBusiness favoritesService;
...
//		return ResponseEntity.notFound().build();
		List<Favorite> favorites = favoritesService.findByEmail(email);
...
//		return ResponseEntity.notFound().build();
		...
		int count = favoritesService.countByProductId(id);
	    if (count > 0) {
...
//		}
//		return ResponseEntity.notFound().build();
		Favorite favorite = favoritesService.findByProductAndUser(productId, email);
	    if (favorite != null) {
...
//		return ResponseEntity.ok(favoriteRepository.save(favorite));
		Favorite savedFavorite = favoritesService.saveFavorite(favorite);
...
//		}
//		return ResponseEntity.notFound().build();
		if (favoritesService.deleteFavoriteById(id)) {
	        return ResponseEntity.ok().build();
	    }
	    return ResponseEntity.notFound().build();
	}
